...
uint8_t displayTrigger = 0;//按键切换显示
...
void drawPage() {
 ...
 if((throttle == 512)&&(displayTrigger == 0))//巡航模式下，滚轮回中，按下一次按键进入((txSettings.triggerMode ==1)&&(throttle == 512)&&(triggerActive()))
{
      if(triggerActive())
    {
      displayData++;
